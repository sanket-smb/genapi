...
def get_item_details():
	...
	for item in master_items_data:
		...
		"total_qty":get_total_stock_qty_in_all_wh(item.get("item_code"))
		}
		items_json.append(items_att)
	return items_json

@frappe.whitelist()
def get_item_data(page_limit,page_number, item_group=None, item_code=None, last_update_on=None, has_stock=None):

	#Adding item_group
	filter = ''
	if (item_group and frappe.db.exists('Item Group', item_group)) or item_code or last_update_on:
		filter = ' WHERE '
